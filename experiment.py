...
from train import LexurnTrainer, run_experiment
from evaluation import compare_models
from dataset import create_dataloaders

def run_lexurn_experiment(config_path="configs/dummy.config", 
                         train_steps=2000, 
                         test_steps=256, 
                         num_epochs=10,
                         save_results=True):
    ...
    print(f"  Normal model relative divergence: {normal_results['rel_div_mean']:.4f} ± {normal_results['rel_div_std']:.4f}")
    print(f"  Lexical model relative divergence: {lex_results['rel_div_mean']:.4f} ± {lex_results['rel_div_std']:.4f}")
    ...
    if rel_div_diff < -0.1:
        result_interpretation = "✅ HYPOTHESIS CONFIRMED: Lexical invariance improves generalization"
        success = True
    elif rel_div_diff > 0.1:
        result_interpretation = "❌ HYPOTHESIS REJECTED: Lexical invariance worsens generalization"  
        success = False
    else:
        result_interpretation = "⚠️  HYPOTHESIS UNCLEAR: Mixed or no effect from lexical invariance"
        success = None
    ...
    if normal_results['rel_div_mean'] > 0.6:
        print(f"  Normal model: MEMORIZING (high relative divergence)")
    elif normal_results['rel_div_mean'] < 0.4:
        print(f"  Normal model: GENERALIZING (low relative divergence)")
    else:
        print(f"  Normal model: MIXED behavior")
        
    if lex_results['rel_div_mean'] > 0.6:
        print(f"  Lexical model: MEMORIZING (high relative divergence)")
    elif lex_results['rel_div_mean'] < 0.4:
        print(f"  Lexical model: GENERALIZING (low relative divergence)")  
    else:
        print(f"  Lexical model: MIXED behavior")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="configs/dummy.config", help="Config file")
