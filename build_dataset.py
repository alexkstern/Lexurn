...
def generate_dataset(
    ...
    urns = generate_urns(n_tasks=n_tasks, n_colors=n_colors, alpha=alpha, device=device, seed=seed)
    ...
    for i in range(n_steps):
        ...
        samples = torch.distributions.Categorical(cat_probabilities).sample(torch.Size([context_len]))
...
if __name__== "__main__":
    ...
    dataset,urns,task_ids=generate_dataset(n_tasks=d_tasks,context_len=context_length,n_colors=n_colors, n_steps=32)
