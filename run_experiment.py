...
def test_new_task_generalization(normal_model, lex_model, config, device='cpu', n_test_tasks=10):
    """
    ...
    with torch.no_grad():
        for task_idx in range(n_test_tasks):
            ...
            context_sequence = context_dist.sample((eval_context_len,))  # (context_len-1,)
            
            # Prepare input for models
            context_input = context_sequence.unsqueeze(0).to(device)  # (1, context_len-1)
            
            # Get model predictions
            normal_logits = normal_model(context_input)  # (1, context_len-1, vocab_size)
    ...
    print(f"Normal Model KL vs Ground Truth:  {normal_kl_mean:.4f} ± {normal_kl_std:.4f}")
...
def run_lexurn_experiment(config_path):
    ...
    if n_tasks == 1:
        print("→ SINGLE-TASK EXPERIMENT: Testing memorization vs generalization on one urn")
    else:
...
def main():
    ...
    results = run_lexurn_experiment(config_path)
